...
db.execute("CREATE VIEW IF NOT EXISTS localhistory AS SELECT strftime('%Y-%m-%d, %H:%M:%S',history.time,'localtime') AS localtime, history.account,history.amount FROM history ORDER BY history.time")
